...
Add to ID token:: Include groups in ID tokens.
Add to access token:: Include groups in access tokens.
Add to lightweight access token:: Include groups in lightweight access tokens.
Add to userinfo:: Include groups in the UserInfo endpoint response.
Add to introspection:: Include groups in the Introspection endpoint response.
Add group role mappings:: When enabled, realm and client roles assigned to the user's organization groups are included in the token for each organization. See <<oidc-token-with-roles,Token structure with roles>> for the output format.
. Click *Save*.
...
Add group role mappings:: When enabled, realm and client roles assigned to the user's organization groups are included as additional SAML attributes. See <<saml-assertion-with-roles,SAML assertion structure with roles>> for the output format.
