...
a|* `getTriggerEventType()`: The event provider ID that triggered activation (e.g., `user-created`, `scheduled`, or `AD_HOC` for manual activation).
...
a|* `getReason()`: The reason for deactivation.
...
a|* `getStepId()`: The ID of the scheduled step.
...
a|* `getSourceWorkflowId()` / `getSourceWorkflowName()`: The source workflow.
